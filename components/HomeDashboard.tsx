...
export default function HomeDashboard() {
  ...
  useEffect(() => {
    ...
    async function load(current: Session | null) {
      ...
      setProgress(map); setLoading(false);
    }
  ...
  async function signOut() { await getSupabase().auth.signOut(); window.location.href = "/"; }
  ...
  return <main className="home-shell">
    <header className="home-header">
      ...
      {session ? <div className="account-block"><span className="live-badge"><i /> {detectiveName.toUpperCase()}</span>{adFree && <span className="adfree-badge">AD-FREE</span>}<button onClick={signOut}>SIGN OUT</button></div> : <Link href="/login" className="header-login">SIGN IN / SIGN UP</Link>}
    </header>
