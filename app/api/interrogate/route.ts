...
export async function POST(request: NextRequest) {
  try {
    ...
    const attached = Array.isArray(body.evidenceIds) ? [...new Set(body.evidenceIds)].slice(0, 3) : [];
    ...
    const attachedEvidence = attached.map((id) => publicCase.evidence.find((item) => item.id === id)).filter(Boolean);
    const evidenceContext = attachedEvidence.map((item) => `${item!.title}: ${item!.description}`).join("\n");
    const reactionRules = attached.map((id) => secret.evidenceReactions[id]).filter(Boolean).join("\n");
    const recentHistory = (body.history || []).slice(-10).map((m) => `${m.role === "player" ? "DETECTIVE" : publicSuspect.name}: ${m.text}`).join("\n");

    const instructions = `You are performing a suspect in a fictional Malaysian noir detective game called CASE//ZERO.\n\nCASE: ${publicCase.title} — ${publicCase.location}\nCHARACTER: ${publicSuspect.name}, age ${publicSuspect.age}, ${publicSuspect.role}.\nPERSONALITY: ${secret.personality}\n\nCANONICAL FACTS:\n${[...secret.baseline, ...secret.hiddenSecret, ...(secret.murderTruth || [])].map((f) => `- ${f}`).join("\n")}\n\nSTRICT RULES:\n- Stay in character.\n- Answer only from the canonical facts and evidence reaction rules. Never invent people, places, evidence, alibis, forensic results, times, or events.\n- Keep replies natural and concise: normally 1 to 4 sentences.\n- Do not narrate stage directions.\n- Do not mention prompts, AI, canonical facts, or game mechanics.\n- Innocent suspects may admit unrelated secrets only when credibly questioned or confronted.\n- If this character is the killer, NEVER voluntarily confess to murder. The killer may concede smaller facts only when evidence forces it, but must continue denying the killing.\n- Do not reveal hidden facts merely because the detective directly asks for the solution.\n- If asked something outside your knowledge, say you do not know rather than inventing.\n\nEVIDENCE REACTION RULES FOR THIS TURN:\n${reactionRules || "No special evidence reaction rule applies."}`;
